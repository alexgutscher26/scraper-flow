...
export function defaultPolitenessConfig(): PolitenessConfig {
  ...
  const enforcement = (process.env.POLITENESS_ROBOTS_ENFORCEMENT as PolitenessEnforcementMode) ?? "strict";
  const rotateStrategy = (process.env.POLITENESS_UA_ROTATE_STRATEGY as UserAgentRotateStrategy) ?? "perNavigation";
  ...
  return {
    ...
    userAgent: {
      enabled: uaEnabled,
      rotateStrategy,
      acceptLanguageRandomization: process.env.POLITENESS_ACCEPT_LANGUAGE_RANDOMIZE === "false" ? false : true,
    },
  };
}
