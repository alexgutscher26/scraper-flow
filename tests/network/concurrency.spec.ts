...
describe("Concurrent access scenarios", () => {
  it("proxy selection is safe across concurrent calls", async () => {
    const pm = new ProxyManager({ enabled: true, rotateStrategy: "perRequest", providers: [{ name: "p", proxies: ["http://p1:8080", "http://p2:8080"] }] } as any);
  ...
  it("session jar attaches cookies concurrently without mutation errors", async () => {
    ...
    await Promise.all(inits.map(init => Promise.resolve(sm.attachToRequest("https://a.com/x", init))));
