...
describe("chrome executable path resolution", () => {
  ...
  it("auto-detects on Windows when env var not set", async () => {
    vi.spyOn(fs, "existsSync").mockImplementation((p: any) => typeof p === "string" && p.includes("Program Files") && p.endsWith("chrome.exe"));
  ...
  it("auto-detects on macOS when env var not set", async () => {
    vi.spyOn(fs, "existsSync").mockImplementation((p: any) => p === "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome");
  ...
  it("throws when Chrome cannot be found", async () => {
    ...
    await expect(resolveChromeExecutablePath("linux")).rejects.toThrow(/Chrome executable not found/);
  });
});
