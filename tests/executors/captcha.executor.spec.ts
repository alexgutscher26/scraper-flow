...
describe('CaptchaExecutor', () => {
  ...
  it('detects reCAPTCHA and verifies token', async () => {
    await page.setContent(
      `<div class='g-recaptcha' data-sitekey='SITEKEY'></div><textarea id='g-recaptcha-response'>tok123</textarea>`
    );
    const inputs = {
      'Web page': page,
      SecurityLevel: 'medium',
      CaptchaType: 'auto',
      CaptchaCredential: JSON.stringify({ apiKey: 'secret' }),
    };
  ...
  it('detects hCaptcha and verifies token', async () => {
    await page.setContent(
      `<div class='h-captcha' data-sitekey='HSITE'></div><textarea name='h-captcha-response'>tok456</textarea>`
    );
