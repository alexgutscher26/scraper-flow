...
function env(endpoint: string) {
  ...
  return {
    getInput: (n: string) => ({ "Endpoint URL": endpoint, Query: "query { ok }", "Variables JSON": "{}", "Use browser context": "false" }[n]),
    setOutput: vi.fn(),
    getPage: () => undefined,
...
describe("GraphQL executor failover", () => {
  it("retries with new proxy on error", async () => {
    ...
    globalThis.fetch = vi.fn(async (input: any, init: any) => {
      ...
      return { ok: true, status: 200, headers: new Headers({ 'content-type': 'application/json' }), json: async () => ({ ok: true }) } as any
    }) as any
