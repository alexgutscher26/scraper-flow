...
describe("HttpClient", () => {
  it("caches JSON within TTL", async () => {
    ...
    const fetchSpy = vi.spyOn(global, "fetch" as any).mockResolvedValueOnce(new Response(JSON.stringify({ ok: true }), { headers: { "content-type": "application/json" } }))
  ...
  it("retries transient errors", async () => {
    ...
    const fetchSpy = vi.spyOn(global, "fetch" as any)
      .mockResolvedValueOnce(new Response("service down", { status: 503, headers: { "content-type": "text/plain" } }))
  ...
  it("emits metrics", async () => {
    ...
    const fetchSpy = vi.spyOn(global, "fetch" as any).mockResolvedValueOnce(new Response(JSON.stringify({ ok: true }), { headers: { "content-type": "application/json", "content-length": "12" } }))
