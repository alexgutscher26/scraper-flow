...
const apiKeyRegex =
  /\b(?:api[_-]?key|x[-_]api[-_]key|authorization|bearer|token|secret|client[-_]secret)\b[^\n]*?(:|=)\s*([^\s,'";]+)/gi;
...
export function sanitizeObject(value: any): any {
  ...
  for (const k of Object.keys(value)) {
    ...
    const keySensitive =
      /password|secret|token|api[-_]?key|authorization|mrn|ssn|account|iban|routing/i.test(k);
