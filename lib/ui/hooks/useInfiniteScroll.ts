...
export function useInfiniteScroll(onReach: () => void, opts: Options = {}) {
  ...
  useEffect(() => {
    ...
    const handle = () => {
      ...
      const scrollHeight = root ? (root as HTMLElement).scrollHeight : document.documentElement.scrollHeight
    ...
    return () => { target.removeEventListener("scroll", handle, optsListener) }
