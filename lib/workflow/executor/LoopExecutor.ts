...
export async function LoopExecutor(
  ...
  try {
    ...
    environment.log.info(`Starting loop iteration over ${parsedArray.length} items`);
    ...
    for (let i = 0; i < parsedArray.length; i++) {
      ...
      environment.log.info(
        `Processing item ${i + 1}/${parsedArray.length}: ${JSON.stringify(currentItem)}`
      ); // Set outputs for current iteration
      environment.setOutput(
        'Current item',
        typeof currentItem === 'object' ? JSON.stringify(currentItem) : String(currentItem)
      );
      environment.setOutput('Current index', i.toString());
      environment.setOutput('Is last', (i === parsedArray.length - 1).toString());
    ...
    environment.log.info(`Loop completed successfully. Processed ${parsedArray.length} items`);
