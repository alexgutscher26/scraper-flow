...
export async function GenerateSelectorExecutor(
  ...
  try {
    ...
    if (credentials) {
      try {
        ...
        const resp = await openai.chat.completions.create({
          model: "gpt-4o-mini",
          messages: [
            { role: "system", content: "Given HTML and a description, propose robust CSS and XPath selectors. Return JSON {selectors: [{type, selector}]} only." },
        ...
        const aiCandidates: CandidateSelector[] = aiSelectors
          ...
          .map((s: any) => ({ selector: s.selector, type: s.type, score: 3.5, metrics: { uniqueness: 0.5, stability: 1, performance: 1, matchCount: 0 }, source: "ai" }));
    ...
    candidates = rerankWithOverride(candidates, overrideSel ? { selector: overrideSel } : undefined);

    const validations: any[] = [];
    for (const c of candidates) {
      const resHtml = validateAgainstHtml(html, c);
      validations.push({ stage: "html", selector: c.selector, type: c.type, valid: resHtml.valid, matched: resHtml.matched });
      if (page) {
        const resPage = await validateAgainstPage(page, c);
        validations.push({ stage: "page", selector: c.selector, type: c.type, valid: resPage.valid, matched: resPage.matched, latencyMs: resPage.selector.metrics.latencyMs });
      }
    }

    const primary = candidates.find((c) => {
      const vHtml = validations.find((v) => v.stage === "html" && v.selector === c.selector && v.type === c.type);
    ...
    const primaryValidation = validations.find((v) => v.stage === "html" && v.selector === selected.selector && v.type === selected.type);
    if (!primaryValidation?.valid) {
      const fallback = candidates.find((c) => validations.find((v) => v.stage === "html" && v.selector === c.selector && v.type === c.type && v.valid));
    ...
    environment.setOutput("Primary selector", JSON.stringify({ selector: selected.selector, type: selected.type }));
    environment.setOutput("Fallback selectors", JSON.stringify(fallbacks.map((f) => ({ selector: f.selector, type: f.type }))));
    environment.setOutput("Selector report", JSON.stringify({ candidates: candidates.map((c) => ({ selector: c.selector, type: c.type, score: c.score, metrics: c.metrics })), validations }));
