...
async function downloadWithBrowser(
  ...
  try {
    ...
    environment.log.info(`Browser fetch successful, base64 length: ${base64Data.length}`);
...
export async function DownloadFileExecutor(
  ...
  try {
    ...
    if (urlType === 'direct') {
      ...
      environment.log.info(`Using direct URL: ${downloadUrl} with filename: ${filename}`);
    } else {
      // Link selector
      try {
        ...
        try {
          ...
          environment.log.error(`Invalid URL extracted from element: ${downloadUrl}`);
          return false;
        }
        ...
        environment.log.error(`Failed to extract download URL: ${error.message}`);
        return false;
      }
    }
    ...
    if (!binaryData) {
      environment.log.error(`Direct fetch failed: no data`);
      ...
      try {
        const browserBuffer = await downloadWithBrowser(page, downloadUrl, environment);
        if (browserBuffer && browserBuffer.length > 0) {
          buffer = browserBuffer;
          environment.log.info(`Browser-based download successful, size: ${buffer.length} bytes`);
        } else {
          environment.log.error('Browser-based download also failed');
          return false;
        }
      } catch (browserError: any) {
        environment.log.error(`Browser-based download failed: ${browserError.message}`);
        return false;
      }
    } else {
      try {
        const arrayBuffer = binaryData as ArrayBuffer;
        environment.log.info(`ArrayBuffer received, size: ${arrayBuffer.byteLength} bytes`);
    ...
    try {
      ...
      environment.log.info(`File data converted to base64 (${fileData.length} characters)`);
      if (fileData.length === 0) {
        environment.log.error('Base64 conversion resulted in empty string');
        return false;
      }
    } catch (error: any) {
      environment.log.error(`Failed to convert file to base64: ${error.message}`);
