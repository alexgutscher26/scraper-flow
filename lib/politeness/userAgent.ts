...
function computeUA(config: PolitenessConfig, state: PolitenessState, url?: string) {
  const pool = config.userAgent.pool && config.userAgent.pool.length > 0 ? config.userAgent.pool : DEFAULT_UA_POOL;
...
export async function applyHeaders(page: Page, config: PolitenessConfig, state: PolitenessState, url?: string) {
