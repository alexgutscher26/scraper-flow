  ...
  async request<T = any>(input: string, init: RequestInit & RequestOptions = {}): Promise<T> {
    ...
    const doFetch = async (): Promise<T> => {
      ...
      while (true) {
        ...
        emitHttpMetric({ url: input, method, status: response.status, durationMs: duration, sizeBytes: sizeHeader ? Number(sizeHeader) : undefined, timestamp: Date.now() })
    ...
    const promise = doFetch().finally(() => { this.inflight.delete(key) })
    this.inflight.set(key, promise)
    return promise
  }

  get<T = any>(url: string, opts: RequestOptions = {}) { return this.request<T>(url, { ...opts, method: "GET" }) }
